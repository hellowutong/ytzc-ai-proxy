...
**开发原则**: 后端能完成的逻辑必须后端完成，前端尽可能减少逻辑.目标是如果以后更换前端技术都能方便迁移.**谨记！**
**工作原则**: 任何时候开始执行工作前都必须要列出具体的工作任务列表然后再开始工作**必须谨记！**
**回复语言**: 必须使用中文回答 
---
...
```
skills 的目录结构为 ./skills/{system,custom}/{category}/{version}/{skill_name}/
...
### 5.1 设计文档约束
**⚠️ 任何设计变更**
- 前端的所有设计变更后,当我提示"手动测试通过"必须更新到 `./docs/frontend_design.md` **严格遵循，谨记！**
- 后端的所有设计变更后,当我提示"手动测试通过"必须更新到 `./docs/backend_design.md`**严格遵循，谨记！**
- 接口的所有设计变更后，当我提示"手动测试通过"必须更新到 `./docs/api_design.md`**严格遵循，谨记！**
...
**⚠️ 业务逻辑修改 **
- 每次业务逻辑修改必须把修改项写如对应的docs文件 **严格遵循，谨记！**
- 每次修改业务逻辑完成后自动使用docker-compose部署前后端 **严格遵循，谨记！**

**⚠️ 测试案例**
- 当我提示"手动测试通过",必须严格对照本次修改的代码编写正确可用的回测的测试案例 必须严格对照本次修改的代码编写正确可用的回测的测试案例
- 每次我手动测试完成后提示你添加测试案例，必须严格对照本次修改的代码编写正确可用的回测的测试案例
- 案例编写完毕必须检查是否有过时的案例，并且明确提示我! **严格遵循，谨记！**
### 5.2 手动测试流程

**⚠️ 如果我说"手动测试"，必须先自动部署！谨记 docker-compose，完成了通知我可以开始。**

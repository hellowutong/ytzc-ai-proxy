...
#### 右栏：AI对话区（文章阅读区与AI对话区通过标签切换）

**布局说明**:
- 文章阅读区与AI对话区共享右栏空间
- 通过顶部标签切换: [📖 文章] [🤖 AI对话]
- 默认显示文章阅读区
- 切换到AI对话区时，自动加载当前文章作为上下文
...
**标签栏样式**（深色主题）:
```
.tab-bar: background: #1e1e1e, border-bottom: 1px solid #333
.tab-item: color: #858585, padding: 12px 16px
.tab-item.active: color: #007acc, border-bottom: 2px solid #007acc
.tab-item:hover: color: #cccccc
```
...
┌─────────────────────────────────────────────────────┐
│ 文章完整标题                                         │
│ 少数派 · 2026-02-24 14:00                            │
│ [打开原文]  [💬 与AI对话]  ← 点击切换到AI对话区      │
└─────────────────────────────────────────────────────┘
```
...
│  📄 当前上下文:                                      │
│  《文章标题...》                                     │
│  来源: 少数派 | 作者: XXX | 2026-02-24              │
└─────────────────────────────────────────────────────┘
```

**上下文横幅样式**（深色主题）:
```
.context-banner: background: #252526, 
...
**快捷按钮样式**（深色主题）:
```
.quick-actions: display: flex, gap: 8px, padding: 12px
.quick-btn: background: #3c3c3c, 
            color: #cccccc,
            border: 1px solid #454545,
            border-radius: 4px,
            padding: 6px 12px
.quick-btn:hover: background: #007acc, color: #fff
.quick-btn.active: background: #264f78
```
...
.message-user: background: #264f78, 
                ...
                margin-left: auto
.message-ai: background: #252526,
               color: #cccccc,
               border-radius: 8px 8px 8px 0,
               border-left: 3px solid #007acc
.message-avatar: font-size: 20px
.message-time: color: #858585, font-size: 12px
```

**输入区域**（底部）:
```
┌─────────────────────────────────────────────────────┐
│  💬 输入消息... (Shift+Enter换行)        [发送]    │
│                                                     │
│  支持:                                              │
│  • Enter直接发送                                    │
│  • Shift+Enter换行                                  │
...
**输入区域样式**（深色主题）:
```
.input-area: background: #252526, 
             border-top: 1px solid #333
.input-textarea: background: #3c3c3c,
                  color: #cccccc,
                  border: 1px solid #454545
.send-btn: background: #007acc, color: #fff
.send-btn:disabled: background: #3c3c3c
```
...
| 功能 | 说明 |
|------|------|
| **模型选择** | 下拉选择当前启用的虚拟模型，默认使用文章关联的虚拟模型 |
| **清空对话** | 清空当前文章的对话历史 |
| **设置按钮** | 打开对话设置抽屉（temperature、max_tokens等） |
| **快捷操作** | 总结/翻译/关键点/提问 四个快捷按钮 |
| **流式响应** | AI回复支持打字机效果逐字显示 |
| **停止生成** | AI生成过程中可点击停止 |
| **对话历史** | 滚动查看历史消息 |
...
| 状态 | 说明 |
|------|------|
| **对话存储** | 与WebChat使用相同的store（useChatStore），对话数据保存在后端MongoDB |
| **上下文加载** | 切换AI对话区时，自动加载文章标题+内容作为system message |
| **模型关联** | 对话绑定到文章ID和模型名称 |
| **历史恢复** | 重新选中文章时，自动恢复之前的对话历史 |
...
阅读区显示文章内容
    ↓
用户点击[💬 与AI对话] 或 点击[🤖 AI对话]标签
    ↓
切换到AI对话区
    ↓
自动发送system message: 文章标题+内容(前3000字)
    ↓
用户点击快捷按钮 或 输入问题
    ↓
调用 /proxy/ai/v1/chat/completions API
    ↓
流式显示AI回复
    ↓
保存消息到对话历史
...
| 快捷按钮 | Prompt模板 |
|----------|-----------|
| **总结** | "请用中文总结这篇文章的主要内容，不超过200字。文章标题: {title}\n\n文章内容: {content}" |
| **翻译** | "请将这篇文章翻译成英文。文章标题: {title}\n\n文章内容: {content}" |
| **关键点** | "请提取这篇文章的3-5个关键要点，用 bullet points 形式列出。文章标题: {title}\n\n文章内容: {content}" |
| **提问** | 用户自定义问题，自动附带文章上下文 |

**上下文长度控制**:

```
- 默认加载文章前3000字作为上下文
- 超过3000字显示提示: "文章较长，已加载前3000字"
- 提供[加载全文]按钮，点击后发送完整文章内容
```
...
├── media.ts            # 媒体文件队列
├── rss.ts              # RSS订阅和文章
└── chat.ts             # 对话管理（WebChat与RSS共用）
```

**重要设计原则**: WebChat与RSS共用`useChatStore`，所有对话统一存储在MongoDB

### 4.1 chat.ts Store设计 (扩展)

**文件位置**: `frontend/src/stores/chat.ts`
...
import type { Conversation, Message, RSSArticle } from '@/types'
...
export const useChatStore = defineStore('chat', () => {
  ...
  async function createConversation(
    model: string, 
    metadata?: Record<string, any>  // 新增可选参数
  ) {
    try {
  ...
  // 扩展: 根据文章获取或创建对话
  async function getOrCreateRSSConversation(
    article: RSSArticle, 
    model: string
  ) {
    // 1. 查找现有对话
    const existing = conversations.value.find(
      c => c.metadata?.source === 'rss' && c.metadata?.article_id === article.id
    )
    ...
    if (conversation) {
      // 3. 自动发送system message加载文章内容
      const systemContent = buildArticleContext(article)
      await sendSystemMessage(conversation.id, systemContent)
    }
  ...
  async function executeQuickAction(
    action: 'summarize' | 'translate' | 'keypoints',
    article: RSSArticle
  ) {
    const prompts = {
    ...
    // 获取或创建对话
    const conversation = await getOrCreateRSSConversation(
      article, 
      article.virtual_model || 'demo1'
    )
  ...
  // 扩展: 构建文章上下文
  function buildArticleContext(article: RSSArticle): string {
    const content = article.content?.slice(0, 3000) || ''  // 默认前3000字
...
**关键设计说明**:
- `createConversation`新增`metadata`参数，**向后兼容**
- `getOrCreateRSSConversation`自动关联文章和对话
- 所有对话通过`fetchConversations`统一获取，WebChat自动显示RSS对话
- 快捷操作通过标准`sendMessage`实现，无需新API

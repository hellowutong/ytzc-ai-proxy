...
| 元素 | 类型 | 功能 |
|------|------|------|
| **Skill名称** | 输入框 | 可修改Skill名称 |
| **代码编辑区** | Monaco Editor | 编辑SKILL.md，YAML frontmatter + Markdown |
| **校验按钮** | 按钮 | ✓ 校验YAML格式、必填字段、JSON Schema |
| **校验状态** | 状态文本 | 🔍 待校验 / ✅ 已通过 / ❌ 有错误 / ⚠️ 有警告 |
| **保存按钮** | 按钮 | 校验通过后才可用，保存到当前版本或创建新版本 |
| **预览按钮** | 按钮 | Markdown实时预览 |
| **版本管理** | 按钮组 | 切换/删除/新增版本 |
...
4. 校验内容：YAML格式、必填字段(name/version/type)、JSON Schema有效性、语法检查
...
│ ✅ 必填字段完整                              │
│ ⚠️ 警告: input_schema 缺少 description      │
│ ❌ 错误: output_schema.type 值无效           │
│                                             │
│ 详细错误:                                    │
│ Line 15: type = "int", 应为 "integer"       │
│                                             │
│        [关闭]  [定位到错误行]                │
└─────────────────────────────────────────────┘
```
